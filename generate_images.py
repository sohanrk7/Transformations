...
def semantic_callback(image, data_dict):
    ...
    data_dict['semantic_images'] = np.reshape(np.copy(image.raw_data), (image.height, image.width, 4))
...
def main():
    ...
    try:
        ...
        camera_bp.set_attribute('image_size_y', '480') # 3x4 aspect ratio supported by Carla on Windows
        camera_bp.set_attribute('fov', '90') #field of view = 90º
        ...
        #opens a live stream
        camera_sem.listen(lambda image: semantic_callback(image, camera_data))
        ...
        while img_counter < 10000:
            ...
            if distance_progress > 0.1:
                previous_time = current_time

            if time_at_same_location > 60:
                print('car immobile or stuck in traffic')
                break

            rgb_im = camera_data['rgb_image']
            sem_im = camera_data['sem_image']
            image_h = cv2.hconcat(rgb_im, sem_im)
            cv2.imshow('2 cameras', image_h)
            if cv2.waitKey(1) == ord('q'):
                break
            if distance_progress > 5:
                img_counter+=1
                time_grab = time.time_ns()
                cv2.imwrite('C:/CARLA_0.9.15/WindowsNoEditor/TrainingData/RGB' %time_grab, rgb_im)
                cv2.imwrite('C:/CARLA_0.9.15/WindowsNoEditor/TrainingData/Sem' %time_grab, sem_im)
                previous_position = hero_car.get_transform()
                ...
                if img_counter % 100 == 0:
                    weather = carla.WeatherParameters(
                        ...
                        fog_density = random.choice([0.0, 0.2, 0.4, 0.4, 0.95]),
